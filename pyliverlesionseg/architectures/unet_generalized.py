...
def create_unet_like_model(
    ...
    :param number_input_features:
    :param subsample_factors_per_pathway:
    :param kernel_sizes_per_pathway:
    :param number_features_per_pathway:
    :param kernel_sizes_common_pathway:
    :param number_features_common_pathway:
    :param dropout_common_pathway:
    :param output_size:
    :param metadata_sizes:
    :param metadata_number_features:
    :param metadata_dropout:
    :param metadata_at_common_pathway_layer:
    :param padding:
    :param pooling:
    :param upsampling:
    :param activation:
    :param activation_final_layer:
    :param kernel_initializer:
    :param batch_normalization:
    :param batch_normalization_on_input:
    :param instance_normalization:
    :param instance_normalization_on_input:
    :param relaxed_normalization_scheme:
    :param mask_output:
    :param residual_connections:
    :param dense_connections:
    :param add_extra_dimension:
    :param l1_reg:
    :param l2_reg:
    :param verbose:
    :return:
    ...
    #Load here, otherwise we get double loading of keras on windows with multiprocessing
    import numpy as np
    from keras.models import Model
    from keras.layers import Input, Dropout, MaxPooling3D, Concatenate, Multiply, Add, Reshape, Lambda, AveragePooling3D
    from keras.layers.convolutional import Conv3D, UpSampling3D, Cropping3D
    ...
    def normalization_function():
        if batch_normalization_on_input or batch_normalization:
            ...
            normalization_function_ = Lambda(lambda x: (x - K.mean(x, axis=[1, 2, 3], keepdims=True)) / (K.std(x, axis=[1, 2, 3], keepdims=True) + K.epsilon()))
    ...
    def introduce_metadata(path, i):
        ...
        for m_n_f, m_d in zip(metadata_number_features[i], metadata_dropout[i]):
        ...
        metadata_path = UpSampling3D(K.int_shape(path)[1:4])(metadata_path)
    ...
    def pooling_function():
        if pooling is "max":
            pooling_function_ = MaxPooling3D
        elif pooling is "avg":
            pooling_function_ = AveragePooling3D
        return pooling_function_

    def upsampling_function():
        if upsampling is "copy":
            upsampling_function_ = UpSampling3D
        elif upsampling is "linear":
            def upsampling_function_(size):
                def TriLinear(path):
                    path = UpSampling3D(size)(path)
                    path = AveragePooling3D(size, strides=(1, 1, 1), padding='valid')(path)
                    return path
                return TriLinear
    ...
    supported_upsamplings = ["copy", "linear"]
    ...
    for i, (s_f_p_p, k_s_p_p) in enumerate(zip([(1, 1, 1)] + subsample_factors_per_pathway, [[[], []]] + kernel_sizes_per_pathway)):
        ...
        for k_s in k_s_p_p[0]:
            output_sizes -= np.array(k_s) - 1 if padding is 'valid' else 0
    ...
    for i, (s_f_p_p, k_s_p_p) in reversed(list(enumerate(zip([(1, 1, 1), (1, 1, 1)] + subsample_factors_per_pathway[:-1], [[[], []]] + kernel_sizes_per_pathway)))):
        ...
        prev_s_f_p_p = np.array(s_f_p_p)
        output_sizes *= s_f
        output_sizes = output_sizes - s_f + 1 if upsampling is "linear" else output_sizes
    for k_s in kernel_sizes_common_pathway:
        output_sizes -= (np.array(k_s) - 1) if padding is 'valid' else 0
    ...
    if verbose and not [o_s in p_o_s for o_s, p_o_s in zip(output_size, possible_output_sizes)] == [True] * 3:
    ...
    for i, (s_f_p_p, k_s_p_p, n_f_p_p) in enumerate(zip([(1, 1, 1)] + subsample_factors_per_pathway, [[[], []]] + kernel_sizes_per_pathway, [[[], []]] + number_features_per_pathway)):
        s_f = s_f_p_p // prev_s_f_p_p
        prev_s_f_p_p = np.array(s_f_p_p)
        path = pooling_function()(s_f, s_f)(path)
        if i > 0:
    ...
    path = Lambda(lambda x: x, name='main')(path) 
